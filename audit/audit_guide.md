# GIT PROJECT AUDIT GUIDE

## STUDENT: Katerina Kasdanastasi
## REPOSITORY: https://platform.zone01.gr/git/kkasdana/git.git
...
**Verify**: `git log --oneline` (should show: Initial commit, Added shebang, Added comment, Added name variable)
...
**Verify**: `git log --pretty=format:"* %h %ad | %s%d [%an]" --date=short`
...
**Verify**: `git checkout <first-commit>`, `git checkout main~1`, `git checkout main`
...
**Verify**: `git tag` (should show: oops, v1, v1-beta)
...
**Verify**: `git reflog` (should show oops tag), `git show 172c89b` (should show author with email in one commit)
...
**Verify**: `ls lib/hello.sh`, `git log --follow lib/hello.sh`
...
**Expected Answer**: 
- objects/: Stores all Git content (blobs, trees, commits)
- config: Repository-specific configuration
- refs/: References to commits (branches, tags)
- HEAD: Points to current branch

**Verify**: `git rev-parse HEAD`, `git cat-file -t HEAD`, `git cat-file -p HEAD`
...
**Verify**: `git branch -a`, `git log --all --graph --oneline`, `git diff main greet -- Makefile`
...
- **Fast-Forward**: Moving branch pointer forward when no divergent changes exist
...
- [ ] Ask the following question to the student: What is the single git command equivalent to what you did before to bring changes from remote to local main branch?
- [ ] Did the student provide an accurate response?

**Expected Answer**: `git pull` (equivalent to git fetch + git merge)

**Verify**: `cd cloned_hello && git remote -v`, `git branch -a`

---

## Bare Repositories
- [ ] Ask the following question to the student: What is a bare repository and why is it needed?
...
**Expected Answer**: A repository without a working directory, used as a central shared repository for collaboration. Acts as a central hub where developers push/pull changes.

**Verify**: `ls -la hello.git/`, `cd hello && git remote -v` (should show shared remote)
...
```bash
cd /var/home/bbyg/VS\ Code/git/work/hello
...
### Documentation Check:
- [ ] Student provided Project_Walkthrough.md with all commands and outputs
- [ ] Student can access remote repository at https://platform.zone01.gr/git/kkasdana/git.git
- [ ] Commit history shows progression through all exercises
